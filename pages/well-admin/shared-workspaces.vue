<template>
  <div class="flex flex-col">
    <div v-if="adminPortfolioList">
      ...
      <NavBreadcrumbs>
        <WHeadingsHTwo heading="Admin WELL Health-Safety" />
      </NavBreadcrumbs>
      <div class="w-full">
        <WTabsHorizontal
          ref="tabs"
          v-model="currentTab"
          class="-mb-px sm:ml-2 sm:w-96"
          alignment="justify-around"
          :tabs="[
            { name: 'Health-Safety Enrollments' },
      ...
      <FormulateForm
        v-if="currentTab === 'Health-Safety Enrollments'"
      ...
      >
        <div class="overflow-x-scroll">
          <table class="w-full divide-y divide-gray-200">
            <thead>
              <tr v-if="currentTab === 'Health-Safety Enrollments'">
                <th
                ...
                >
                  <div
                    @click="
                      regStatus(portfolio)
                        ? $router.push(
                            `/shared-workspaces/${portfolio.portfolio_number}/dashboard`
                          )
                        : $router.push(
                            `/shared-workspaces/create/${portfolio.portfolio_number}`
                          )
                    "
                  >
            ...
            <tbody v-else class="bg-white divide-y divide-gray-200">
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div
                    v-html="
                      checkValue(project.hsr_aggrement, 'Agreement Not Signed')
                    "
                  >
                    <span v-if="project.hsr_aggrement">{{
...
export default {
  ...
  data() {
    return {
      currentTab: 'Health-Safety Enrollments',
  ...
  head() {
    return {
      title: `Admin WELL Health-Safety`,
    }
  },
  computed: {
    ...mapState({
    ...
    paginate() {
      if (
        this.currentTab === 'Health-Safety Enrollments' &&

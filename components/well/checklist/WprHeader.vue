<template>
  <div class="pb-8">
    ...
    <WStepsHorizontal
      ...
      :steps="[
        {
          name: 'SELECT 21 FEATURES',
          status: getStatStatus,
          caption: yesCount + '/21 selected',
        },
        {
          ...
          caption: totalDocumentCounts + '/21 uploaded',
        },
        {
    ...
    <div class="grid grid-cols-3 mt-5">
        ...
        <div v-if="!recertificationList.length">
          ...
          <div v-else>
            <div v-if="getDocumentCounts === ''">
              ...
              />
              <p class="text-sm text-gray-600 max-w-xl">
                To earn your rating, each location must achieve at least 21 of
          ...
          >
            ...
            <WLinksBase
              class="ml-2"
              target="_blank"
              :link="`https://v2.wellcertified.com/en/performance-rating/overview`"
            >
      ...
      <ChecklistStatCard
        :progress="progress"
        header="Progress"
        :sub-header="21 + ' Required Features'"
        :total-strategies="15"
        :yes="yesCount"
        :maybe="maybeCount"
        :no="noCount"
...
export default {
  ...
  computed: {
    ...
    progress() {
      if (this.scores) {
        if (this.yesCount) {
          return this.yesCount > 21 ? 100 : (this.yesCount / 21) * 100
        }
      }
      return 0
    },

    ...
    >
      ...
      <div class="py-6 space-y-4 border-b-2 border-gray-300">
        <div class="text-gray-600 flex flex-col">
          ...
          <span v-if="!selectedDoc.verifiable.is_auditable" class="pt-1"
        ...
        <div
          v-if="!selectedDoc.verifiable.is_auditable"
          class="-ml-1.5"
        ...
        <div v-if="!selectedDoc.verifiable.is_auditable && showPart">
          <FormulateForm
            :key="resetVerifs"
            class="py-4 px-6 border rounded-md border-gray-400"
            @submit="addNewOption()"
...
export default {
  ...
  computed: {
    ...mapState({
      verificationList: (state) => state.scorecard.verifications,
  ...
  methods: {
    parseIncomingData() {
      ...
      // this.newOption.space_type = this.spaceTypes[0].id

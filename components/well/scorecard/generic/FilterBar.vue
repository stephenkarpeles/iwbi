<template>
  <div class="flex flex-wrap relative">
    <div class="flex relative w-full px-11">
      ...
      <WFeedbackModal v-if="pendingReview" ref="observationModal" class="z-20">
          ...
          >
            ...
            <div class="bg-white sm:rounded-lg sm:shadow mb-4">
              ...
              >
                ...
                <div
                  v-for="concept in conceptObservations"
                  :key="concept.concept"
                >
          ...
          <Popover ref="conceptPopover" y="bottom" x="right">
            ...
            <template #content>
              <div class="p-3 flex w-96">
                <div v-if="project" class="mx-auto my-auto w-full">
                  <FormulateInput
                    v-model="filters.loa"
                    type="select"
                    class="w-full"
                  ...
                  <FormulateInput
                    ...
                    placeholder="Filter by Verification"
                  />
                  <hr class="border-gray-300 mt-2 p-2" />
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      spaceTypes: (state) => state.project.spaceTypes,
    }),
  ...
  watch: {
    ...
    pendingReview(value) {
      if (
        value &&
        value.general_observation_concepts &&
        value.general_observation_concepts.length > 0
      ) {
        value.general_observation_concepts.forEach((e) => {
  ...
  async created() {
    ...
    if (this.project) {
      ...
      if (this.project.project_type === 'cs') {
        this.filters.is_core = true
      }
    ...
    this.$store.dispatch('scorecard/getReviewResponses', {
      module: 'project',
  ...
  methods: {
    ...
    submitObservation() {
      const formData = {
        general_observation: this.generalObservation,
        general_observation_concepts: [],
      }
      ...
      formData.general_observation_concepts = conceptObservations
    ...
    async applyFilters() {
      ...
      try {
        ...
        this.$refs.filterSlider.isHidden = true
      } catch (error) {
        // console.error(error)
      }
    },

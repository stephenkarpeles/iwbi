<template>
  <div>
    <div v-if="doneLoading">
      ...
      >
        <PortfolioScorecardFilterBar
          v-if="
            $route.path.includes('portfolio') ||
            $route.path.includes('projects/v2')
          "
          ref="filter-bar"
          :module="module"
        />
        <ScorecardFilterBar v-else ref="filter-bar" :module="module" />
      </div>
      <div
      ...
      <ScorecardHeader :type="scorecard.type" />
      <div
        v-if="
          pendingReview &&
      ...
      >
        <template #content>
          <div class="px-6 mt-4">
            ...
            <ScorecardAnticipatedScoreIndicator
      ...
      <table class="mt-3 w-full mx-auto shadow divide-y rounded-t-lg">
        <thead>
          <tr>
            ...
            <th
              class="w-3/12 px-6 py-3 bg-primary-700 text-left text-xs leading-4 font-medium text-white uppercase tracking-wider pl-3"
            >
              Weight
            </th>
            <th
        ...
        >
          <tr v-for="feature in features" :key="feature.id">
            ...
            <td colspan="4">
              <LazyScorecardTableRow
                v-for="part in feature.parts"
                :id="'scorecard-part-' + part.id"
                ref="table-row"
                :key="part.concept_name + '-' + part.id"
                :module="module"
...
export default {
  ...
  async mounted() {
    ...
    const spaceTypes = this.module.space_types.map(function (space) {
      return space.id
    })
    ...
    await this.$store.dispatch('project/fetchSpaceTypes')
  ...
  methods: {
    ...
    async downloadReport(format) {
      ...
      if (this.module.type === 'v2') {
        await this.$store
          .dispatch('scorecard/downloadReport', format)
          .then((result) => {
        ...
        await this.$store
          .dispatch('scorecard/downloadScorecardReport', format)
          .then((result) => {
            window.open(result.data.url, '_blank')
            this.reportDownloadLink = result.data.url
          })

<template>
  <div>
    <div v-if="pendingList.data.length">
      <Table>
        <template #header>
        ...
        <template #body>
          <tr v-for="(el, i) in pendingList.data" :key="i">
            <td
              class="whitespace-nowrap font-medium text-primary-700 px-6 py-4"
            >
              {{ el.name }}
            </td>
            <td
              class="whitespace-nowrap font-medium text-gray-500 text-right px-6 py-4"
            >
              File Pending
            </td>
          </tr>
      ...
      <FormInputsPagination
        class="border-none"
        :pagination="pendingList"
        :offset="3"
        @paginate="$parent.getToDoList()"
      />
    </div>

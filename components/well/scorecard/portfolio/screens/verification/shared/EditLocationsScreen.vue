    ...
    <Table v-if="!loading && locations.length" class="my-4 pb-5">
      ...
      <template #body>
        <tr
          ...
          :key="index"
          :class="loc.reviewed ? 'bg-gray-100' : ''"
        >
          <td
          ...
          >
            <FormulateInput
              ...
              :disabled="loc.reviewed ? true : false"
              type="checkbox"
            />
          </td>
          <td
...
export default {
  ...
  methods: {
    ...
    async getLocations() {
      ...
      try {
        const res = await this.$axios.get(
          `api/scorecard/${this.scorecard.id}/documents/${
            this.option.id
    ...
    abort() {
      if (this.isAudit && this.auditLocations.data) {
        this.auditLocations.data = this.auditLocations.data.forEach((el) =>
          el.reviewed || el.assigned
            ? (el.selected = true)
            : (el.selected = false)
        )

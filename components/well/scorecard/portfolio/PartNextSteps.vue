<template>
  <div>
    ...
    <WFeedbackModal ref="modal">
      ...
      >
        ...
        <div class="max-h-72 overflow-auto">
          <PortfolioScorecardDocumentModal
    ...
    <div v-else>
      <div>
        <PortfolioScorecardPartStatusScreen
          v-if="
      ...
      >
        <template #content>
          <div class="p-4">
            <PortfolioScorecardInnovationForm
              :part="part"
              :module-type="moduleType"
              :module="module"
              :form-type="formType"
      ...
      <div class="flex space-x-2">
        <div class="w-1/2">
          <div class="text-lg text-primary-700 font-medium flex h-10">
            ...
            <svg
              v-if="!isReview || currentUser.role.includes('well-admin')"
              ...
              stroke="currentColor"
              @click="
                ;($refs.modal.isHidden = false), (modalMode = 'documents')
              "
            >
              <path
                stroke-linecap="round"
          ...
          >
            ...
            >
              <WButtonsBase
                ...
                "
              >
                <span v-if="part.feature_name.includes('β')">
                  Upload Beta Proposal
                </span>
...
export default {
  ...
  methods: {
    async getFormSubmissions() {
      const mod =
        this.moduleType === 'hsr' || this.moduleType === 'wpr'
          ? 'portfolio'
          : this.moduleType
      await this.$axios
        .get(
          `api/${mod}/${this.module.id}/innovationProposal?part_id=${this.part.id}`
        )
        .then((response) => {
    ...
    async removeDocument(documentId) {
      await this.$store.dispatch('scorecard/removeDocument', documentId)
    ...
    async addDocument() {
      ...
      data.append('part_doc_type', this.featuredocumentType)

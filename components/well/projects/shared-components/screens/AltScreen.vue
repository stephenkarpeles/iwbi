<template>
  <div class="flex flex-wrap md:px-10 sm:px-4 md:py-5">
    ...
    <div class="w-full mb-3 pb-4">
      <WTabsHorizontal
        ref="tabs"
        v-model="currentTab"
        class="-mb-px md:ml-2 md:w-96 pb-4 md:pb-0"
        alignment="justify-around"
        :tabs="[{ name: 'Alternatives' }, { name: 'Additional Guidance' }]"
      />
      <div
        v-if="currentScreen === 'landing'"
        class="md:grid md:grid-cols-2 gap-4 mb-5 md:space-y-0 space-y-4"
      >
        <div
          v-if="currentTab === 'Alternatives'"
          class="flex flex-wrap shadow-lg border rounded-lg md:px-5 px-2 pt-2 pb-5"
        >
          <WHeadingsHFour
            class="w-full"
          ...
          <div
            class="w-full mt-auto flex flex-col md:flex-row md:space-y-0 space-y-2 md:space-x-2 w-80"
          >
            <WButtonsBase
              data-id="submit-aap-btn"
              v-if="project.type !== 'v2-pilot-upgraded'"
              @click.native="currentScreen = 'aap-form'"
            >
              Submit AAP
            </WButtonsBase>
            <WLinksBase
        ...
        >
          ...
          <div
            class="w-full mt-auto flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 w-80"
          >
            <WButtonsBase
              data-id="submit-ep-btn"
              v-if="project.type !== 'v2-pilot-upgraded'"
              @click.native="currentScreen = 'ep-form'"
            >
              Submit EP
            </WButtonsBase>
            <WLinksBase
        ...
        <div
          v-else
          class="shadow-lg border rounded-lg md:px-5 px-2 pt-2 pb-5 col-span-2"
        >
          ...
          <div
            class="w-full mt-auto flex flex-col md:flex-row md:space-y-0 space-y-2 md:space-x-2 md:w-80"
          >
            <WButtonsBase
              data-id="req-guidance-btn"
              v-if="
                project.type !== 'v2-pilot-upgraded' &&
                project.reg_payment_status === 'paid'
              "
              @click.native="handleRequestGuidance"
            >
              Request Guidance
            </WButtonsBase>
            <WLinksBase
      ...
      >
        ...
        <div v-if="verifiedMembership">
          <ProjectGuidanceForm ref="guidance-form" :key="forceReMount + 1" />
        </div>
      ...
      >
        <ProjectStrategiesList />
      </div>
      <div
        v-if="
          currentScreen === 'landing' && currentTab === 'Additional Guidance'
        "
        class="shadow-lg border rounded-lg mt-5"
      >
        <div v-if="currentTicket" class="p-4">
          <WLinksBase
            link="#"
            icon="arrow-narrow-left"
            @click.native="currentTicket = null"
          >
            Back to tickets
          </WLinksBase>
          <ProjectTicketScreen :ticket="currentTicket" />
        </div>
        <ProjectGuidancesList v-else />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async handleRequestGuidance() {
      ...
      this.currentScreen = 'guidance-form'
      await this.$store

              ...
              <tbody class="p-5">
                <tr v-for="item in invoiceDetails.invoice_items" :key="item.id">
                  <td class="border-2 px-2 py-3 text-sm text-right flex">
                    ...
                    >
                      <img
                        :src="$config.assetsURL + 'wpr/wpr-seal-hd.png'"
                ...
                <tr>
                  ...
                  <td
                    class="border-2 px-6 py-3 text-sm text-right font-bold text-primary-600"
                  >
                    ${{ invoiceDetails.total_amount | formatNumber }}
                  </td>
                </tr>
    ...
    getInvoiceInfo() {
      this.$axios
        ...
        .then((response) => {
          ...
          if (
            this.invoiceDetails?.addresses[0]?.organization &&
            this.invoiceDetails?.addresses[0]?.organization !== ''
          ) {
            this.company = {
              ...this.company,
              name: this.invoiceDetails.addresses[0].organization,
            }
          }
          if (this.invoiceDetails?.addresses[0]?.company_id) {
            this.company = {
              ...this.company,
              id: this.invoiceDetails.addresses[0].company_id,
            }
          }
        })
    },

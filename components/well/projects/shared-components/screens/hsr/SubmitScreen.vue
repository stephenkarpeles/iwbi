    ...
    <div>
      <div class="">
        <div class="relative bg-white p-5 rounded-lg">
          ...
          <FormulateForm v-if="pricingDetails" @submit="optHsr('terms')">
            <div class="mb-5">
              <FormulateInput
                v-model="reviewType"
                type="select"
...
export default {
  ...
  mounted() {
    if (this.$store.state.project.project.hsr_opt) {
      this.reviewType = this.$store.state.project.project.hsr_review

          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(strategy, index) in strategies.data" :key="index">
              <td class="px-6 py-4 whitespace-nowrap flex justify-between">
                <nuxt-link
                  class="
                    ...
                    uppercase
                  "
                  :to="`/projects/${project_type}/${strategy.project_number}/alternatives/${strategy.id}`"
                >
                  {{ strategy.type }}-0{{ strategy.id }}
                </nuxt-link>
                <a
                  :href="`/projects/${project_type}/${strategy.project_number}/alternatives/${strategy.id}`"
                  target="_blank"
                >
                  <WIconsBase
                    icon="external-link"
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div
                  class="text-sm leading-5 text-gray-900 flex justify-between"
                >
                  <WLinksBase
                    :link="`/projects/${project_type}/${strategy.project_number}/dashboard`"
                  >
                    {{ strategy.project_number }}
                  </WLinksBase>
                  <a
                    :href="`/projects/${project_type}/${strategy.project_number}/dashboard`"
                    target="_blank"
                  >
                    <WIconsBase
                      icon="external-link"
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm leading-5 text-gray-900">
                  <span v-if="strategy.v1_aap === 1 && strategy.v1feature">{{
                    strategy.v1feature.name
                  }}</span>
                  <span v-else-if="strategy.feature">{{
...
export default {
  ...
  computed: {
    ...mapState({
      project_type: (state) => state.project.project.type,
      project_id: (state) => state.project.project.id,
      strategies: (state) => state.project.strategies,
    }),
    paginate() {
      if (this.strategies) {
        return {
  ...
  mounted() {
    ...
    this.$store.dispatch('project/getStrategies', this.allFilters)
  },
}
</script>

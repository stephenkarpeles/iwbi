    ...
    <div class="lg:absolute lg:inset-0">
      <div class="lg:absolute lg:inset-y-0 lg:right-0 lg:w-1/2">
        <img
          ...
          src="/wpr/enrollment/enrollment-3.png"
          alt=""
        />
      </div>
    </div>
    <div
      class="relative md:pt-8 pb-8 md:pb-16 px-4 sm:pt-16 sm:px-6 lg:px-8 lg:max-w-7xl lg:mx-auto lg:grid lg:grid-cols-2 border rounded-md"
    >
      <div class="lg:pr-8">
        <div v-if="formData" class="max-w-md mx-auto sm:max-w-lg lg:mx-0">
          ...
          <FormulateForm @submit="submitSharedWorkspace">
            ...
            />
            <div
              class="bg-secondary-100 text-secondary-700 border p-2 mb-2 text-sm rounded border-secondary-700"
            >
...
export default {
  data() {
    return {
      ...
      removedSpaces: [
        { label: 'All Spaces', name_plural: 'all Spaces' },
        { label: 'High Humidity Spaces', name_plural: 'high humidity spaces' },
        { label: 'Core and Shell', name_plural: 'core and shell' },
        { label: 'Others', name_plural: 'others' },
        { label: 'Hospitality', name_plural: 'hospitality' },
        { label: 'Assisted Living', name_plural: 'assisted living' },
        {
          label: 'Cultural Institutions',
          name_plural: 'cultural institutions',
        },
        { label: 'Public Assembly', name_plural: 'public assembly' },
        { label: 'Industrial', name_plural: 'industrial' },
        { label: 'Fitness', name_plural: 'fitness' },
        { label: 'Sporting Venue', name_plural: 'sporting venue' },
        { label: 'Healthcare', name_plural: 'healthcare' },
        { label: 'Childcare', name_plural: 'childcare' },
        { label: 'Auditorium', name_plural: 'auditorium' },
        { label: 'Transportation', name_plural: 'transportation' },
        { label: 'Library', name_plural: 'library' },
        { label: 'Grocery', name_plural: 'grocery' },
        { label: 'Shopping Center', name_plural: 'shopping center' },
        { label: 'Manufacturing', name_plural: 'manufacturing' },
        { label: 'Medical Office', name_plural: 'medical office' },
        { label: 'Data Centers', name_plural: 'data centers' },
        { label: 'Convention Centers', name_plural: 'convention centers' },
        { label: 'Airport', name_plural: 'airport' },
        { label: 'Sports/entertainment', name_plural: 'ports/entertainment' },
        { label: 'Venue', name_plural: 'venue' },
        { label: 'Hotels', name_plural: 'hotels' },
      ],
    }
  },
  computed: {
    spaceTypes() {
      if (this.$store.state.shared.spaceTypes) {
        // filter fetched spaces against spaces requested for removal from the wpr create list
        let array1 = this.$store.state.shared.spaceTypes
        const array2 = this.removedSpaces

        array1 = array1.filter(
          (i) => !array2.map((j) => j.label).includes(i.label)
        )
        return array1
      }
      return null

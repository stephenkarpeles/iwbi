<template>
  <div class="relative bg-white rounded-lg shadow-md">
    ...
    >
        ...
        <div class="space-y-4">
          ...
          <div v-if="invoiceSplitData">
            ...
            <ul class="list-desc ml-5 max-w-2xl">
              <li v-for="(data, index) in invoiceSplitData" :key="index">
                #<span
                  class="text-primary-500 cursor-pointer hover:text-primary-800"
                  @click="openInvoiceInNewTab(data.invoice_number)"
                >
                  {{ data.invoice_number }}
                </span>
                - {{ data.description }}
          ...
          <div v-else>
            ...
            <div v-if="splitData.type === 'percentage'" class="w-96 mt-2">
              <div
      ...
      <div
        v-if="!invoiceData.user_preference"
        class="px-4 py-5 border-b border-gray-200 sm:px-6 md:flex justify-between"
      >
        <div class="flex">
          <WHeadingsHTwo
            :heading="
        ...
        <div class="md:flex space-x-0 md:space-x-4">
          <span
            class="mb-3 border border-primary-300 rounded flex items-center px-2 py-2"
          >
            <WLinksBase
              class="w-full flex justify-around"
              icon="question-mark-circle"
              link="https://well.support/2ad2b012-5fc0-45bb-a953-7a40872b2f14#mdl-4822"
              target="_blank"
            />
          </span>
          <div class="md:w-40 mb-3 md:mb-0">
            <WButtonsBase
          ...
          <div v-if="currentUser" class="md:w-40">
            <WButtonsBase
              v-if="
                invoiceData.status === 'unpaid' ||
                currentUser.role.includes('well-admin')
              ...
              type="primaryInverted"
              @click.native="
                $store.dispatch('shared/setActiveTab', 'FEES + DISCOUNTS')
              "
            >
              Back
            </WButtonsBase>
      ...
      <SharedEnrollmentCheck
        v-if="invoiceData.user_preference"
        :invoice="invoiceData"
      />
      <div v-if="!editMode && !invoiceData.user_preference">
        <div class="md:flex flex-wrap px-5 md:px-8 py-8">
          <div class="md:w-1/3">
            <dl class="grid grid-cols-1 gap-3 md:gap-6">
              <div>
          ...
          <div class="md:w-2/3">
            <div class="md:grid grid-cols-3 gap-4">
              <div class="mt-3 md:mt-0 col-start-3">
        ...
        <div
          class="overflow-x-scroll md:overflow-hidden mx-2 flex flex-wrap px-0 md:px-8"
        >
          <div class="w-full">
            <div class="flex">
              <div
                class="w-1/2 px-3 md:px-6 py-3 bg-primary-50 text-left text-xs leading-4 font-medium text-primary-500 uppercase tracking-wider"
              >
                DESCRIPTION
              </div>
              <div
                class="md:w-1/5 px-3 md:px-6 py-3 bg-primary-50 text-left text-xs leading-4 font-medium text-primary-500 uppercase tracking-wider text-center"
              >
                QUANTITY
              </div>
              <div
                class="w-1/5 px-3 md:px-6 py-3 bg-primary-50 text-left text-xs leading-4 font-medium text-primary-500 uppercase tracking-wider text-right"
              >
                PRICE
              </div>
              <div
            ...
            >
              <div
                class="w-1/2 px-3 md:px-6 py-4 text-sm leading-5 font-medium text-gray-900 overflow-hidden"
              >
                {{ item.description }}
                <div
                  ...
                  class="md:flex"
                >
                  <span v-if="invoiceData.discount_code" class="text-sm pr-2">
              ...
              <div
                class="w-1/5 px-3 md:px-6 py-4 text-sm leading-5 text-gray-500 text-center"
              >
                {{ item.quantity }}
              </div>
              <div
                class="w-1/5 px-3 md:px-6 py-4 text-sm leading-5 text-gray-500 text-right break-words"
              >
              ...
              >
                {{
                  (item.unit_cost * item.quantity).toFixed(2) | formatCurrency
                }}
                USD
              </div>
            </div>
        ...
        <div
          ...
          "
          class="max-w-3xl mx-auto mt-10 rounded-md bg-yellow-50 p-4 mb-5 rounded-lg border border-yellow-300"
        >
          <div class="flex">
        ...
        <div
          :class="[
            ...
            invoiceData.status === 'unpaid' &&
              ...
              invoiceData.v1_project_standard !== 'community' &&
              'opacity-0',
            'md:flex flex-wrap px-5 md:px-8 pb-8',
          ]"
        >
          <div class="md:w-2/3">
            <div v-if="paymentType === 'card'" class="">
              ...
              <div
                :class="[
                  ...
                  'flip-card duration-700 transition-transform transform relative h-52 mt-6 mx-auto w-72 md:w-96',
                ]"
              >
                <div
                      ...
                      <div class="flex-1 space-y-4 py-1">
                        <div class="space-y-2">
                          ...
                          <div
                            ...
                            class="flex text-xl md:text-2xl text-primary-300 items-center"
                          >
            ...
            <div
              ...
              class="bg-white p-5 md:mx-5 my-5 shadow sm:rounded-lg sm:px-6 divide-y divide-gray-200"
            >
          ...
          <div class="md:w-1/3">
                ...
                <div class="flex">
                  ...
                  <div class="ml-3">
                    <div class="text-sm leading-5 text-red-700">
                      <p>Something went wrong: {{ errorCode }}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="mt-5">
              ...
              >
                ...
                >
                  <span v-if="!processingPayment"
                    >Pay now and Complete enrollment</span
                  >
                  <span v-else>
                    <WLoadingSpinner type="button" class="mx-auto" />
                  </span>
              ...
              <div v-if="paymentType && paymentType != 'card'">
                <WButtonsBase @click.native="setUserPreference">
                  Download invoice and Complete enrollment
                </WButtonsBase>
              </div>
            </div>
          </div>
...
export default {
  ...
  computed: {
    ...
    activeAddress() {
      if (this.sharedWorkspace.billing_org === this.sharedWorkspace.owner_org) {
        return this.sharedWorkspace.addresses.find((a) => a.is_shipping)
      }
      return this.sharedWorkspace.addresses.find((a) => a.is_billing)
    },
    isChinaInvoice() {
  ...
  mounted() {
    if (this.allInvoices && this.allInvoices.length) {
      this.getInvoiceData()
    }
  ...
  methods: {
    async getInvoiceData() {
      await this.$store
        .dispatch(
          ...
          this.allInvoices[0].invoice_number
        )
        .then((res) => {
    ...
    async splitInvoice() {
      ...
      data.invoice_id = this.invoiceData.id
    ...
    verifyDiscount() {
      if (this.discountCode) {
        this.$axios
          .get(
            `api/invoice/${this.invoiceData.id}/apply-discount/${this.discountCode}`
          )
          .then((resp) => {
            this.$refs.alert.showAlert(
              'success',
          ...
          .catch((error) => {
            this.errorCode = error.response.data.msg
          })
      }
    },
    setupStripe() {
        ...
        await this.stripe
          ...
          .then((result) => {
            if (result.error) {
              ...
              this.$axios
                .post('api/invoice/' + this.invoiceData.id + '/payment', {
    ...
    async setUserPreference() {
      await this.$store.dispatch('invoice/updateUserFieldsinInvoice', {
        ...
        email: this.invoiceData.user_email,

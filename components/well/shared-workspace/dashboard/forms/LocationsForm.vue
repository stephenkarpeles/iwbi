  ...
  <div class="w-full">
    <FormulateForm class="my-2" @submit="submitForm">
      ...
      <div :class="{ hidden: tab !== 'Required' }">
        ...
        >
          * Locations must share the same space type to be eligible for the same
          enrollment. Please begin another enrollment for locations with
          separate or distinct uses.
        </p>
        <FormulateInput
          v-if="
      ...
      <div class="flex justify-end">
        <div class="w-48">
          ...
          <FormulateInput
            v-else
            :disabled="apiLoading || spaceTypeNoMatch"
            class="col-start-6"
            type="submit"
            label="Submit"
          />
...
export default {
  ...
  computed: {
    ...
    spaceTypeNoMatch() {
      if (
        this.recertification?.location_changed === 1 &&
        this.formData.primary_space_type_id.toString() !==
          this.sharedWorkspace.space_types[0].id.toString()
      ) {
        return true
      } else {
        return false
  ...
  async mounted() {
    ...
    if (this.projectId) {
      ...
      // In HSR Renewal + Locations Confirmation mode, default space type to Original Space Type
      if (this.recertification && this.recertification.location_changed === 1) {
        this.formData.primary_space_type_id =
          this.sharedWorkspace.space_types[0].id.toString()
      }
    }
  },
  methods: {
    ...
    async submitForm() {
      // In HSR Renewal + Locations Confirmation mode, space type MUST match original space type. Prevent submit otherwise.
      if (

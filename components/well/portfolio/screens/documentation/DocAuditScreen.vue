  ...
  <div>
    ...
    >
      ...
      <FormInputsPagination
        ...
        :pagination="scorecardAuditDocuments"
        :offset="3"
...
export default {
  ...
  methods: {
    getProjectDetails(document) {
      const targetDocument = document.project_ids.filter(
        (doc) => doc.id === document.project_id
      )[0]

      if (targetDocument) {
        return `${targetDocument.project_number} - ${targetDocument.name}`
      } else {
        return '-'
      }
    },
    ...
    async getScorcard() {
      if (this.portfolio && this.portfolio.scorecard_id !== null) {
        await this.$store.dispatch('portfolio/getScorecardAuditDocuments', {
          ...
          page: this.scorecardAuditDocuments.current_page,
          filterShared: this.filterScorecardList,
          type: ['Individual-Scale', 'Audit-Document'],
          name: this.$parent.documentSearch,
        })
      }
    },

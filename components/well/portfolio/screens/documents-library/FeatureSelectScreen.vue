...
export default {
  ...
  watch: {
    verification() {
      ...
      if (!this.isAudit) this.getNonAuditList()
      else this.getPartList()
    },
  },
  async mounted() {
    if (!this.isAudit) await this.getNonAuditList()
    else await this.getPartList()
  },
  methods: {
    async getPartList() {
      ...
      if (this.portfolio.type === 'v2') {
        payload = {
          verifications: this.verification ? [this.verification] : [],
        }
      } else {
        payload = {
      ...
      await this.$axios
        ...
        .then((resp) => {
          this.listData = resp.data.map((e) => {
            const subpartIds = [
              ...new Set(
                e.verifications[0].options.map((item) => item.subpart_id)
              ),
            ]
            return {
              part_prefix: e.prefix,
              name: e.name,
              concept_name: e.concept_name,
              id: e.id,
              sub_parts: subpartIds.map((subPartId) => {
                return {
                  ...e.verifications[0].options.find(
                    (a) => a.subpart_id === subPartId
                  ),
                  id: subPartId,
                  sub_part_id: subPartId,
                  options: e.verifications[0].options.filter(
                    (a) => a.subpart_id === subPartId
                  ),
                }
              }),

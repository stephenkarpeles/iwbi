<template>
  <div class="lg:max-h-96 lg:h-[600px] overflow-x-auto overflow-y-auto pr-2">
    ...
    <div :class="mode === 'locations' && 'hidden'" class="px-6">
      ...
      <div class="py-6 space-y-4 border-b-2 border-gray-300">
        <div class="text-primary-800 flex flex-col">
          <span class="text-xl font-bold">Selected feature part(s)</span>
        ...
        <PortfolioScoreFeatureSelectScreen
          :part="part"
          :option="option"
      ...
      <div class="py-6 space-y-4 border-b-2 border-gray-300">
        <div class="text-primary-800 flex flex-col">
          <span class="text-xl font-bold">File upload</span>
        </div>
      ...
      <div class="py-6 space-y-4">
        <div class="text-primary-800 flex flex-col">
          <span class="text-xl font-bold">Add a note</span>
...
export default {
  ...
  computed: {
    ...
    scorecardPartId() {
      return this.options
        ? this.options.find((el) => el.option_id === this.option.id)?.id
        : ''
    },
    verificationOptions() {
  ...
  mounted() {
    ...
    this.getSelectedOptions(this.part)
  },
  ...
  methods: {
    ...
    formatFormData() {
      ...
      if (this.docType === 'Audit-Document') {
        data.append('scorecard_part_id', this.option.scorecard_part_id)
      } else {
    ...
    async updatePartState() {
      await this.$parent.$parent.getVerifications()
    },
    async getSelectedOptions(part) {
      try {
        const { data } = await this.$axios.get(
          `api/scorecard/${this.scorecard.id}/part/${part.id}/optionSelectionLists`
        )
        this.options = data
      } catch (error) {}
    },

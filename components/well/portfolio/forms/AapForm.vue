    ...
    <div v-if="features">
      ...
      <div>
        <FormulateForm class="mt-5 space-y-2" @submit="submitForm">
          <FormulateInput
            v-if="portfolio && portfolio.hsr_scorecard_version_id !== null"
...
export default {
  ...
  computed: {
    ...
    aapType() {
      const label = this.portfolio.type === 'v2' ? 'WELL v2' : 'WELL v2 Pilot'
      return [
        { label, value: this.portfolio.type },
  ...
  mounted() {
    if (this.portfolio && !this.strategy) {
      this.$store.dispatch(
  ...
  methods: {
    getFeaturesByType() {
      if (this.formData.type_internal === 'v2-hsr') {
        this.$store.dispatch(
          'features/getAllFeaturesByVersionId',
          this.portfolio.hsr_scorecard_version_id
        )
      } else {
        this.$store.dispatch(
          'features/getAllFeaturesByVersionId',
    ...
    submitForm() {
      ...
      if (!this.strategy) {
        this.$store
          .dispatch('portfolio/createAAP', this.formData)
          .then(this.clearForm(), (this.$parent.currentScreen = 'landing'))
      } else {
        this.$store.dispatch('portfolio/updateAAP', this.formData).then(() => {
          this.clearForm()
          setTimeout(() => {
            this.$store.dispatch(
